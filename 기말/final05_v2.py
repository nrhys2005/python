...
# 이벤트를 처리하는 함수
def fire(event):
    if(len(bullets)==2 or crash==1):
        return
    else:
        bullet = Ball(canvas, "red", 10, 150, 250, 10, 0, img=bullet_img)
...
im = Image.open('alien.png').resize((50,50 ))
enemy_image = ImageTk.PhotoImage(im)
enemy=Ball(canvas, 'red', 50,500,200,0,5,img=enemy_image)
...
while True:
    if(crash==0):
        for bullet in bullets:
            ...
            if(isRectangleOverlap(bullet.rect(),enemy.rect())):
                crash=1
